...
class PivxTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave bitcloudds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop bitcloudds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing bitcloudd/bitcloud-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitcloudds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a bitcloudd test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple bitcloudd test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'bitcloudd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "bitcloudd should have exited with an error"
                else:
                    assert_msg = "bitcloudd should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitcloudd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "bitcloudd"), "-spendzeroconfchange=1", "-server", "-keypool=1",
...
class ComparisonTestFramework(PivxTestFramework):
    ...
    Sets up some bitcloudd binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "bitcloudd"),
                          help="bitcloudd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "bitcloudd"),
                          help="bitcloudd binary to use for reference nodes (if any)")
