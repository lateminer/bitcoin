...
versions of Tor see [Section 3](#3-automatically-listen-on-tor).*
	...
	HiddenServicePort 8329 127.0.0.1:8329
	HiddenServicePort 61472 127.0.0.1:61472

The directory can be different of course, but (both) port numbers should be equal to
your bitcloudd's P2P listen port (8329 by default).
	...
	-externalip=X   You can tell bitcloud about its publicly reachable address using
	                ...
	                addresses are not supported by the Bitcloud network). Given the above
...
and open port 8329 on your firewall (or use -upnp).
...
in `torrc` config file. For cookie authentication the user running bitcloudd must have read
access to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running bitcloudd to the same group and setting permissions appropriately. On
Debian-based systems the user running bitcloudd can be added to the debian-tor group,
