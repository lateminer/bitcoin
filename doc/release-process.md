...
* Update manpages, see [gen-manpages.sh](https://github.com/LIMXTEC/Bitcloud/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
### Bitcloud maintainers/release engineers, suggestion for writing release notes
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in bitcloud, then:
...
### Build and sign Bitcloud Core for Linux, Windows, and macOS:

    pushd ./gitian-builder
    ./bin/gbuild --num-make 2 --memory 3000 --commit bitcloud=v${VERSION} ../bitcloud/contrib/gitian-descriptors/gitian-linux.yml
    ...
    ./bin/gbuild --num-make 2 --memory 3000 --commit bitcloud=v${VERSION} ../bitcloud/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make 2 --memory 3000 --commit bitcloud=v${VERSION} ../bitcloud/contrib/gitian-descriptors/gitian-osx.yml
...
Build output expected:
  ...
  4. macOS unsigned installer and dist tarball (`bitcloud-${VERSION}-osx-unsigned.dmg`, `bitcloud-${VERSION}-osx64.tar.gz`)
...
Codesigner only: Sign the macOS binary:
    ...
    transfer bitcloud-osx-unsigned.tar.gz to macOS for signing
...
- Detached signatures will then be committed to the [bitcloud-detached-sigs](https://github.com/LIMXTEC/Bitcloud-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
bitcloud-${VERSION}-riscv64-linux-gnu.tar.gz
...
- Announce the release:
  ...
  - Create a [new GitHub release](https://github.com/LIMXTEC/Bitcloud/releases/new) with a link to the archived release notes.
