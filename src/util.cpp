...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bitcloud.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty bitcloud.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override bitcloud.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitcloudd.pid"));
