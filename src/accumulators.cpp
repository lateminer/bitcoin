...
bool CalculateAccumulatorCheckpoint(int nHeight, uint256& nCheckpoint, AccumulatorMap& mapAccumulators)
{
    ...
    if (nHeight % 10 != 0) {
        nCheckpoint = pblockindex->nAccumulatorCheckpoint;
        return true;
    }
    ...
    bool fFilterInvalid = nHeight >= Params().Zerocoin_StartHeight();
...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators)
{
    ...
    //It is VERY IMPORTANT that when this is being run and height < v2_start, then zPIV/zGROW need to be disabled at the same time!!
...
std::list<CBlockIndex*> calculateAccumulatedBlocksFor(
    ...
    while (pindex) {
        ...
        // 10 blocks were accumulated twice when zPIV/zGROW v2 was activated
