                       ...
                       int &nPaymentCount, int &nBlockStart, CBitcoinAddress &address, CAmount &nAmount)
{
    ...
    if (nAmount < 10 * COIN)
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid amount - Payment of %d is less than minimum 10 BTDX allowed", FormatMoney(nAmount)));
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"bitcloud-address\" monthy-payment\n"
            ...
            "5. \"bitcloud-address\":   (string, required) Bitcloud address to send payments to\n"
    ...
    CWalletTx wtx;
    if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 50 BTDX collateral for proposal
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"bitcloud-address\" monthly-payment \"fee-tx\"\n"
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) Bitcloud address of payment\n"
