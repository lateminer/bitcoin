...
void UpdateTime(CBlock* pblock, const CBlockIndex* pindexPrev)
{
    ...
    if (Params().AllowMinDifficultyBlocks())
        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock, pblock->IsProofOfStake());
}
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    unsigned int nBlockMaxSizeNetwork = pblock->nTime > (unsigned int)Params().Zerocoin_StartTime() ? MAX_BLOCK_SIZE_CURRENT : MAX_BLOCK_SIZE_LEGACY;
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zPIVs/zGROWs that have been in mempool long
                    //and higher priority to zPIVs/zGROWs that are large in value
                    ...
                    // zPIV/zGROW spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // ToDo: replace afer Zerocoin activation
            // unsigned int nMaxBlockSigOps = MAX_BLOCK_SIGOPS_CURRENT;
            unsigned int nMaxBlockSigOps = GetAdjustedTime() > Params().Zerocoin_StartTime() ? MAX_BLOCK_SIGOPS_CURRENT : MAX_BLOCK_SIGOPS_LEGACY;
            ...
            // double check that there are no double spent zPIV/zGROW spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zPIV/zGROW serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (!fProofOfStake && pblock->nTime > Params().StartMasternodePayments()) {
        ...
        if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
            //For the period before v2 activation, zPIV/zGROW will be disabled and previous block's checkpoint is all that will be needed
