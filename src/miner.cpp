...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zBTDX spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zBTDX spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zBTDX serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
{
    LogPrintf("BitcloudMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("bitcloud-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BitcloudMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("BitcloudMiner exception");
    } catch (...) {
        LogPrintf("BitcloudMiner exception");
    }

    LogPrintf("BitcloudMiner exiting\n");
}
