...
const string strMessageMagic = "GROW Signed Message:\n";
...
{
    //Check to see if the zPIV/zGROW is properly signed
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    // Size limits
    // ToDo: replace after Zerocoin activation
    // unsigned int nMaxSize = MAX_ZEROCOIN_TX_SIZE;
    unsigned int nMaxSize = GetAdjustedTime() < Params().Zerocoin_StartTime() ? MAX_BLOCK_SIZE_LEGACY : MAX_ZEROCOIN_TX_SIZE;
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            // Check that zPIV/zGROW mints are not already known
        ...
        if (!tx.IsZerocoinSpend()) {
            ...
            unsigned int nMaxSigOps = GetAdjustedTime() > Params().Zerocoin_StartTime() ? MAX_TX_SIGOPS_CURRENT : MAX_TX_SIGOPS_LEGACY;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from PIVX/GROW
...
void RecalculateZPIVSpent()
{
    ...
    while (true) {
        ...
        //Add mints to zPIV/zGROW supply
        for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        //Remove spends from zPIV/zGROW supply
...
{
    // PIVX/GROW: recalculate Accumulator Checkpoints that failed to database properly
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    unsigned int flags = SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG |
                         SCRIPT_VERIFY_DERKEY | SCRIPT_VERIFY_LOW_S;
    ...
    unsigned int nMaxBlockSigOps = block.nTime > (unsigned int)Params().Zerocoin_StartTime() ? MAX_BLOCK_SIGOPS_CURRENT : MAX_BLOCK_SIGOPS_LEGACY;
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert PIV/GROW to zPIV/zGROW
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (block.GetBlockTime() > Params().Zerocoin_StartTime()) {
        if ((block.nVersion < Params().Zerocoin_HeaderVersion() || block.nVersion >= (int)0x20000000) && Params().NetworkID() != CBaseChainParams::REGTEST)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && block.GetBlockTime() > FutureDrift(GetAdjustedTime()))
    ...
    unsigned int nMaxBlockSize = GetAdjustedTime() > Params().Zerocoin_StartTime() ? MAX_BLOCK_SIZE_CURRENT : MAX_BLOCK_SIZE_LEGACY;
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zPIV/zGROW spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (block.GetBlockTime() <= pindexPrev->GetPastTimeLimit() && (Params().NetworkIDString() != "regtest")) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // PIVX/Phore/GROW: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zero", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zero", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
