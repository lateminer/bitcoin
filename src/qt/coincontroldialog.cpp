...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::BTDX;
    ...
    QString toolTip4 = tr("Can vary +/- %1 ubtdx per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (haveDest) {
                ...
                // if listMode or change => show Bitcloud address. In tree mode, address is not shown again for direct wallet address outputs
