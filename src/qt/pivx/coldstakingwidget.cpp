    ...
    ui(new Ui::ColdStakingWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your BTDXs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept BTDX delegation / Delegate BTDX"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BTDX outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 BTDX" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
