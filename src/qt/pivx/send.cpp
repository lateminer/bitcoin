    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("BTDX");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zBTDX");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (BTDX) or private coins (zBTDX)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Bitcloud address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BTDX");
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(isChecked ? tr("Send BTDX") : tr("Send zBTDX"));
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zBTDX is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zBTDX selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zBTDX transaction sent!"));
        ...
        if (receipt.GetStatus() == ZPIV_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zBTDX require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zBTDX transaction
        ...
        Q_EMIT message("zBTDX transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isPIV) {
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zBTDX in your balance to select."));
        }
    }
}
