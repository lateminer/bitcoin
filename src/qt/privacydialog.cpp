                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zPIV/zGROW ought to be enough for anybody." - Bill Gates, 2017
...
void PrivacyDialog::on_pushButtonSpendzPIV_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, send zPIV/zGROW
        sendzPIV();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPIV/zGROW
    sendzPIV();
}
...
void PrivacyDialog::sendzPIV()
{
    ...
    // Warn for additional fees if amount is not an integer and change as zPIV/zGROW is requested
    ...
    // use mints from zPIV/zGROW selector if applicable
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zPIV/zGROW was spent successfully update the addressbook with the label
    ...
    // Clear zPIV/zGROW selector in case it was used
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonMintzPIV->setToolTip(tr("zGROW is currently disabled due to maintenance."));

        // Spend zPIV/zGROW
        ui->pushButtonSpendzPIV->setEnabled(false);
        ui->pushButtonSpendzPIV->setToolTip(tr("zGROW is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zPIV/zGROW
        ui->pushButtonMintzPIV->setEnabled(true);
        ui->pushButtonMintzPIV->setToolTip(tr("PrivacyDialog", "Enter an amount of GROW to convert to zGROW", 0));
