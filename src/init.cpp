...
void PrepareShutdown()
{
    ...
    util::ThreadRename("bitcloud-shutoff");
...
{
    util::ThreadRename("bitcloud-loadblk");
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Bitcloud Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Bitcloud Core is probably already running."), strDataDir));
