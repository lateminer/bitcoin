...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 40420, 30420));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:40420"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The GROW Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that PIVX/GROW is running in a usable environment with all
...
#endif
    // Make sure only a single PIVX/GROW process is using the data directory.
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //PIVX/GROW specific: zerocoin and spork DB's
                ...
                // PIVX/GROW: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        //Inititalize zPIVWallet/zGROWWallet
        uiInterface.InitMessage(_("Syncing zGROW wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZPivBackups = GetBoolArg("-backupzgrow", true);
...
//    }
//
//    nAnonymizePivxAmount = GetArg("-anonymizegrowamount", 0);
