...
// Blackcoin stake modifier (hash modifier of proof-of-stake):
// The purpose of stake modifier is to prevent a txout (coin) owner from
// computing future proof-of-stake generated by this txout at the time
// of transaction confirmation. To meet kernel protocol, the txout
// must hash with a future stake modifier to generate the proof.
...
bool CheckStakeProtocolV2(uint256 nStakeModifierV2, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, const uint256& bnTarget,
                uint256& hashProofOfStake)
{
    ...
    CDataStream ss(SER_GETHASH, 0);
    ss << nStakeModifierV2 << txPrev.nTime << prevout.hash << prevout.n << nTimeTx;
...
bool CheckStakeNewProtocol(const CDataStream& ssUniqueID, CAmount nValueIn, const uint64_t nStakeModifier, const uint256& bnTarget,
                unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    // Get stake weight - weight is equal to coin amount
...
bool CheckStakeKernelHash(CStakeInput* stakeInput, unsigned int nBits, unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    for (int i = 0; i < nHashDrift; i++) //iterate the hashing
    {
        ...
        if (nTimeTx >= Params().NewKernelProtocolTime()) {
            ...
            // Old protocol
            // if (!CheckStakeProtocolV2(nStakeModifier2, txPrev, txin.prevout, tx.nTime, stake->GetValue(), bnTargetPerCoinDay, hashProofOfStake))
                // continue;
        }
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (nTxTime >= Params().NewKernelProtocolTime()) {
        ...
        if (!CheckStakeNewProtocol(stake->GetUniqueness(), stake->GetValue(), nStakeModifier, bnTargetPerCoinDay, blockprev.nTime,
        ...
        if (!CheckStakeProtocolV2(nStakeModifierV2, txPrev, txin.prevout, tx.nTime, bnTargetPerCoinDay,
                        hashProofOfStake)) {
            return error("CheckProofOfStake() : INFO: check kernel failed on coinstake %s, hashProof=%s \n",
                        tx.GetHash().GetHex(), hashProofOfStake.GetHex());
        }
